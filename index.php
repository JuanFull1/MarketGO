...
// Conexión protegida
try {
  // Use MySQL PDO provider from db.php
  $pdo = getPDO();
  // Diagnóstico conexión
  try {
    // MySQL diagnostics
    $info = $pdo->query("SELECT DATABASE() AS db, USER() AS usr")->fetch();
    debug('Conexion MySQL', $info ?: []);
  } catch (Throwable $e) {
  ...
  try {
    $tblCheck = $pdo->prepare("
      ...
      WHERE table_name = 'perfil_usuario' AND table_schema = DATABASE()
      LIMIT 1
    ");
    $tblCheck->execute();
    $tableExists = (bool) $tblCheck->fetchColumn();
    if (!$tableExists) {
      $errMsg = "La tabla 'perfil_usuario' no existe. Ejecuta el script SQL en esta BD.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error)) {
  try {
    if ($action === 'login') {
      ...
      if (!$user || !$password)
        throw new Exception("Completa usuario/correo y contraseña.");
...
>>>>>>> e7e90fad002d08f2f3d1b675819258015becfd25
      ...
      $_SESSION['uid'] = $row['id']; // UUID or numeric id
      $_SESSION['uname'] = $row['nombre']; // nombre mostrado
      $role = $row['rol_sistema'] ?? null;
      // Redirigir según rol
      if ($role === 'administrador') {
        header("Location: admin_panel.php");
      } elseif ($role === 'moderador') {
    ...
    if ($action === 'register') {
      ...
      $chkMail = $pdo->prepare("SELECT 1 FROM perfil_usuario WHERE correo = ? LIMIT 1");
      ...
      $ins = $pdo->prepare("
        ...
        VALUES
          (:nombre, :apellido, :cedula, :correo, :telefono, :direccion, :genero, :username, :hash, 1, 1)
      ");
      $ins->execute([
        ':nombre' => $nombre,
        ':apellido' => $apellido,
  ...
  <main class="hero">
    <div class="container hero-inner">
      <div>
        ...
        <p class="hero-sub">Explora productos, chatea con vendedores y administra tus ventas — todo en un solo lugar.
        </p>
