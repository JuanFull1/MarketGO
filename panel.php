...
body{font-family:system-ui,Segoe UI,Roboto;background:var(--bg);color:#17212B}
...
.card{background:var(--card);border-radius:16px;box-shadow:var(--shadow);border:1px solid #eef2f7;margin-bottom:20px}
...
.table th,.table td{padding:8px;border-bottom:1px solid #eef2f7;text-align:left;vertical-align:middle}
.badge{padding:4px 8px;border-radius:999px;font-size:.75rem;font-weight:700;display:inline-block}
.badge-warn{background:#FFF5D9;color:#8a6d00;border:1px solid #ffe9a6}
.badge-info{background:#E8F1FF;color:#174ea6;border:1px solid #cfe0ff}
            ...
            <td>
              <div class="stack">
                ...
                <?php else: ?>
                  ...
                  <a class="btn btn-danger" href="producto_eliminar.php?id=<?= $r['id'] ?>" onclick="return confirm('¿Eliminar publicación?')">Eliminar</a>
                <?php endif; ?>
              </div>
            </td>
          </tr>
          <?php endforeach; if(!$misRows): ?>
            <tr><td colspan="6">Sin publicaciones.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
      ...
      <?php foreach($savRows as $p): ?>
        <a href="producto_detalle.php?id=<?= $p['id'] ?>" style="text-decoration:none;color:inherit">
          <div class="card" style="overflow:hidden">
            <img src="<?= htmlspecialchars($p['imagen_portada_url'] ?: 'https://placehold.co/800x600?text=Producto') ?>" alt="" style="width:100%;aspect-ratio:4/3;object-fit:cover">
            <div class="section">
